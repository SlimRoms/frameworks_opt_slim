...
public class ExpandableCardAdapter extends RecyclerView.Adapter<ExpandableCardAdapter.ViewHolder> {
    ...
    public void setCards(ArrayList<ExpandableCard> cards) {
        mCards.clear();
        mCards.addAll(cards);
    ...
    private void updateExpand(ViewHolder holder, boolean expanded, boolean animate) {
        if (animate)
            TransitionManager.beginDelayedTransition((ViewGroup) holder.itemView.getParent());
        holder.screenshot.setVisibility(expanded ? View.VISIBLE : View.GONE);
        if (animate)
            holder.expandButton.animate().rotation(expanded ? -180 : 0);
        else
            holder.expandButton.setRotation(expanded ? -180 : 0);
    }
