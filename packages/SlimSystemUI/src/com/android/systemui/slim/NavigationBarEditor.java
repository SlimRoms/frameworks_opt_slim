...
public class NavigationBarEditor implements View.OnTouchListener {
    ...
    private void selectIconFromIconPack() {
        ...
        //intent.setAction(Intent.ACTION_MAIN);
        intent.setClassName("com.android.systemui", "com.android.systemui.editor.IconPackActivity");
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        mContext.startActivity(intent);
    }
       ...
       ArrayList<ActionConfig> buttons = new ArrayList<>();
        ...
        for (View v : mNavBar.getButtons()) {
            if (v instanceof SlimKeyButtonView) {
                ...
                Log.d("TEST", "config == null ? " + (config == null));
                if (config != null) {
                    Log.d("TEST", "saving button - " + config.getClickAction());
                    buttons.add(config);
                }
            }
        }
